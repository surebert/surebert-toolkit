...
sb.include('strings.stripHTMLComments');
sb.include('strings.stripBlankHTML');
sb.include('strings.stripWordHTML');
sb.include('strings.parseHTML');
...
sb.widget.editor.prototype = {
    ...
    dataOut: function() {
        ...
        return sb.strings.parseHTML(str, this.basicOnly);
    },
    ...
    processCommand: function(command) {
        switch (command) {
            ...
            case 'removeformat':
                try {
                    this.editor.replaceSelection(sb.strings.stripHTML(this.editor.getSelection()));
                } catch (e) {
                }
                break;
    ...
    makeToolBar: function() {
        ...
        if (sb.widget.editor.buttons.bold) {
            this.addButton('bold', '<b>b</b>', 'bold');
        }

        if (sb.widget.editor.buttons.italic) {
            this.addButton('italic', '<i>i</i>', 'italic');
        }

        if (sb.widget.editor.buttons.underline) {
            this.addButton('underline', '<u>u</u>', 'underline');
        }

        if (sb.widget.editor.buttons.superscript) {
            this.addButton('superscript', '<sup>sup</sup>', 'superscript');
        }

        if (sb.widget.editor.buttons.subscript) {
            this.addButton('subscript', '<sub>sub</sub>', 'subscript');
        }

        if (sb.widget.editor.buttons.strikethrough) {
            this.addButton('strikethrough', '<strike>strike</strike>', 'strikethrough');
        }

        if (sb.widget.editor.buttons.horizontalRule) {
            this.addButton('horizontal rule', '<u>hr</u>', 'inserthorizontalrule');
        }
        ...
        if (sb.widget.editor.buttons.unorderedList) {
            this.addButton('make list', 'ulist', 'insertunorderedlist');
        }

        if (sb.widget.editor.buttons.orderedList) {
            this.addButton('make list', 'list', 'insertorderedlist');
        }


        if (sb.widget.editor.buttons.undo) {
            this.addButton('undo last edit', 'undo', 'undo');
        }
        if (sb.widget.editor.buttons.redo) {
            this.addButton('redo last edit', 'redo', 'redo');
        }
        ...
        this.addButton('view html source', 'view source', function() {
            ...
            if (this.box.style.display == 'none') {
                this.document.body.innerHTML = sb.strings.parseHTML(this.textarea.value, this.basicOnly);
                this.textarea.style.display = 'none';
                this.box.style.display = '';
            } else {
                this.textarea.value = sb.strings.parseHTML(this.document.body.innerHTML, this.basicOnly);
